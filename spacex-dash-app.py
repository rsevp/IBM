...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Successful Landings per Launch Site')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]['class'].value_counts().reset_index()
        fig = px.pie(values=filtered_df.iloc[:,1], 
        names=filtered_df.iloc[:,0], 
        title=f'Success (1) on {entered_site}')
        return fig
...
def get_scatter_chart(entered_site,payload_range):
    ...
    if entered_site == 'ALL':
        ...
        title='Payload Mass Success (1) Per Booster Version')
        ...
        fig = px.scatter(filtered_df, y='class', 
        x='Payload Mass (kg)', color="Booster Version Category",
        title=f'Payload Mass Success (1) at {entered_site}')
        fig.update_layout(xaxis_range=payload_range)
        return fig
